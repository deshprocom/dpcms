...
RSpec.describe RaceRank, type: :model do
  ...
  let(:race_rank) { RaceRank.create(race: race, player: player, earning: 1000, score: 10) }
  ...
  describe '创建排名成功' do
    it '应为选择的牌手增加相应的earning与score' do
      ...
      RaceRank.create(race: race, player: player, earning: 1000, score: 10)
      player.reload
      expect(player.dpi_total_earning).to eq(1000)
      expect(player.dpi_total_score).to eq(10)

      RaceRank.create(race: race, player: player, earning: 1000)
      player.reload
