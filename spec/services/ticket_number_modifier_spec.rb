...
RSpec.describe Services::TicketNumberModifier do
  ...
  let!(:ticket_info) { FactoryGirl.create(:ticket_info, race: race, entity_ticket_number: 10) }

  context '当ticket_info lock_version正确时' do
    it '新增电子票成功' do
      e_ticket_number = ticket_info.e_ticket_number
      result = Services::TicketNumberModifier.call(ticket_info, e_ticket_increment: 1)
    ...
    it '减少电子票成功' do
      e_ticket_number = ticket_info.e_ticket_number
      result = Services::TicketNumberModifier.call(ticket_info, e_ticket_decrement: 1)
      expect(result.code).to eq(0)
      expect(e_ticket_number - 1).to eq(ticket_info.reload.e_ticket_number)
    end
    ...
    it '新增实体票成功' do
      ...
      result = Services::TicketNumberModifier.call(ticket_info, entity_ticket_increment: 1)
      expect(result.code).to eq(0)
    ...
    it '减少实体票成功' do
      entity_ticket_number = ticket_info.entity_ticket_number
      result = Services::TicketNumberModifier.call(ticket_info, entity_ticket_decrement: 1)
      expect(result.code).to eq(0)
      expect(entity_ticket_number - 1).to eq(ticket_info.reload.entity_ticket_number)
    end
  end
  ...
  context '修改票数为0时' do
    it '应返回 修改的票数不能为空或为零' do
      ...
      arr.each do |hash|
        result = Services::TicketNumberModifier.call(ticket_info, hash)
  ...
  context '当ticket_info lock_version不正确时' do
    it '新增电子票，应返回修改票数失败' do
      m_ticket_info = TicketInfo.find race.ticket_info.id
